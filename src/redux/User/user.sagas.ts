...
import { GoogleProvider, handleUserProfile } from './../../firebase/utils';
import {
  signInSuccess,
  signOutUserSuccess,
  userError
...
export function* getSnapshotFromUserAuth(user: any, additionalData = {}) {
  try {
    const userRef: unknown = yield call(handleUserProfile, { userAuth: user, additionalData });
    const snapshot: DataSnapshot = yield userRef;
    if (snapshot) {
